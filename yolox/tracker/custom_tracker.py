...
class STrack(BaseTrack):
    ...
    def reset_filter(self):
        for pos in self.trace[-self.maximum_trace:]:
            self.update(STrack(pos, self.score), self.frame_id)


    def update_trace(self, tlwh):
        self.trace.append(tlwh)
        if len(self.trace) > self.maximum_trace:
            self.reset_filter()
...
class BYTETracker(object):
    ...
    def update(self, output_results, img_info, img_size):
        ...
        # discard fuse_score
        # print(f"High association")
        if not self.args.mot20:
            dists = matching.fuse_score(dists, detections)
            # print(f"IOU dist: {dists}")
        ...
        # print(f"Low detection found: {len(dets_second)}")
