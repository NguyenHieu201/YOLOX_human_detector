...
DATA_PATH = 'datasets/supermarket_dataset/'
...
if __name__ == '__main__':
    ...
    for split in SPLITS:
        ...
        for seq in sorted(seqs):
            ...
            print(num_images)
            # image_ids = range(9, 1800, 10)
            # image_ids = range(12, 31, 1)
            # for i in range(num_images):
            image_ids = os.listdir(f"{data_path}/{seq}/img1")
            image_ids = [int(filename.removesuffix(".jpg")) - 1 for filename in image_ids]
            # print(image_ids)
            for i in image_ids:
                # print(i)
                # TODO: xxx
                ...
                image_info = {'file_name': '{}/img1/{:06d}.jpg'.format(seq, i + 1),  # image name.
                              'id': i - min(image_ids) + 2 + image_cnt,  # image number in the entire training set.
            ...
            if split != 'test':
                ...
                for i in range(anns.shape[0]):
                    ...
                    ann = {'id': ann_cnt,
                           ...
                           'image_id': frame_id - min(image_ids) + 2 + image_cnt,
                           'track_id': tid_curr,
                           'bbox': anns[i][2:6].tolist(),
                           'conf': float(anns[i][6]),
                           'iscrowd': 0,
                           'area': float(anns[i][4] * anns[i][5])}
                    # remove not relevant image
                    # if (ann['image_id'] - 1) in image_ids:
